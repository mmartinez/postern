...
func NewTokenCmd(store token.Store, validator Validator, account string) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Capture, inspect, re-validate, and delete the service-account token\n" +
			"postern uses to resolve secret references at request time. The token\n" +
			"is stored in the OS keychain when available and never echoed to\n" +
			"stdout in full.",
	}
...
func newTokenSetCmd(store token.Store, validator Validator, account string) *cobra.Command {
	...
	cmd.Flags().BoolVar(&fromStdin, "stdin", false, "Read token from stdin (single line)")
