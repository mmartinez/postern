...
// Postern can construct, validate, and (in T5) resolve secrets without
// importing the SDK directly. The wrapper isolates the pre-1.0 SDK behind
// a small surface that's easy to fake in tests.
//
// Only the M1 token-broker use case is covered today: Client construction
// from a service-account token and HealthCheck. T5 will extend this package
// with the Resolver interface backed by SecretsAPI.Resolve.
package onepassword
...
// the SDK. PRD §10 requires the registered integration name "Postern".
...
func New(ctx context.Context, token, integrationVersion string) (*Client, error) {
	...
	return &Client{vaults: s.Vaults()}, nil
}

// HealthCheck verifies the configured service-account token is valid and
// the 1Password SaaS is reachable. It calls Vaults.List — the PRD-blessed
// cheap, read-only operation — and returns nil on success or the wrapped
// SDK error on failure.
//
